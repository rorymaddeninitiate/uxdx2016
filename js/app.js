...
.controller('HomeCtrl', function ($http, BLOG_API, BLOG_URL, conference) {
  'ngInject';
  // this.news = news;
  var self = this;
  this.blog = BLOG_URL;
  this.conference = conference.data;
  // this.ghostmail = ghost.url.api('posts', {limit: 3});
  // $http.get(ghost.url.api('posts', {limit: 3}) +'&callback=JSON_CALLBACK').then(function (data) {
  //   self.news = data.data;
  // });

  var request = new XMLHttpRequest();
  request.open('GET', 'https://latest.uxdxconf.com/ghost/api/v0.1/posts/?limit=3&client_id=ghost-frontend&client_secret=35e2d4887693', true);

  request.onload = function() {
    if (request.status >= 200 && request.status < 400) {
      // Success!
      self.news = JSON.parse(request.responseText);
