...
angular.module('constants', [])
  ...
  .value('CONFERENCE_URL', '/conferences/' + CONFERENCE_ID)
  .value('BLOG_URL', BLOG_URL)
  .value('BLOG_API', BLOG_URL + '/ghost/api/v0.1/posts/?limit=3&client_id=ghost-frontend&client_secret=' + BLOG_ID)
...
.controller('HomeCtrl', function ($http, BLOG_API, BLOG_URL) {
  'ngInject';
  // this.news = news;
  var self = this;
  this.blog = BLOG_URL;
  // this.ghostmail = ghost.url.api('posts', {limit: 3});
  $http.get(BLOG_API).then(function (data) {
    self.news = data.data;
  });
